...
class TestRiskProfilesModule(ModuleTestCase):
    ...
    def mock_client(self):
        with patch("ansible_collections.zscaler.ziacloud.plugins.modules.zia_risk_profiles.ZIAClientHelper") as mock_class:
            ...
            client_instance.risk_profiles.add_risk_profile.return_value = (
                MockBox({"id": 1, "name": "test", "whitelist_urls": [], "blacklist_urls": []}),
                None,
                None,
            )
            client_instance.risk_profiles.delete_risk_profile.return_value = (
                MockBox({"id": 1, "name": "test", "whitelist_urls": [], "blacklist_urls": []}),
                None,
                None,
            )
            client_instance.risk_profiles.get_risk_profile.return_value = (
                MockBox({"id": 1, "name": "test", "whitelist_urls": [], "blacklist_urls": []}),
                None,
                None,
            )
            client_instance.risk_profiles.update_risk_profile.return_value = (
                MockBox({"id": 1, "name": "test", "whitelist_urls": [], "blacklist_urls": []}),
                None,
                None,
            )
