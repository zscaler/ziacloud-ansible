...
class TestSandboxAdvancedSettingsModule(ModuleTestCase):
    ...
    def test_state_absent_clears_list(self, mock_client):
        ...
        mock_client.sandbox.get_behavioral_analysis.return_value = (
            MockBox({"fileHashesToBeBlocked": ["abc123def456789012345678901234ab"]}), None, None
        )
    ...
    def test_check_mode_change_needed(self, mock_client):
        ...
        mock_client.sandbox.get_behavioral_analysis.return_value = (
            MockBox({"fileHashesToBeBlocked": []}), None, None
        )
        set_module_args(
            provider=DEFAULT_PROVIDER,
