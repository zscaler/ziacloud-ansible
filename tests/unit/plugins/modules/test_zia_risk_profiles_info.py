...
class TestRiskProfilesinfoModule(ModuleTestCase):
    @pytest.fixture
    def mock_client(self):
        with patch("ansible_collections.zscaler.ziacloud.plugins.modules.zia_risk_profiles_info.ZIAClientHelper") as mock_class:
            mock_class.zia_argument_spec.return_value = REAL_ARGUMENT_SPEC.copy()
            client_instance = MagicMock()
            mock_class.return_value = client_instance

            client_instance.risk_profiles.get_risk_profile.return_value = (MockBox({'id': 1, 'profile_name': 'test'}), None, None)
            client_instance.risk_profiles.list_risk_profiles.return_value = ([MockBox({'id': 1, 'profile_name': 'test'})], None, None)
            yield client_instance

    def test_list_or_get(self, mock_client):
        mock_client.risk_profiles.list_risk_profiles.return_value = ([MockBox({'id': 1, 'profile_name': 'test'})], None, None)
        set_module_args(provider=DEFAULT_PROVIDER, profile_name="test")
        from ansible_collections.zscaler.ziacloud.plugins.modules import zia_risk_profiles_info
        with pytest.raises(AnsibleExitJson) as result:
            zia_risk_profiles_info.main()
