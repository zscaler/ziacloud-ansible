...
class TestDlpNotificationTemplateinfoModule(ModuleTestCase):
    @pytest.fixture
    def mock_client(self):
        with patch("ansible_collections.zscaler.ziacloud.plugins.modules.zia_dlp_notification_template_info.ZIAClientHelper") as mock_class:
            mock_class.zia_argument_spec.return_value = REAL_ARGUMENT_SPEC.copy()
            client_instance = MagicMock()
            mock_class.return_value = client_instance

            client_instance.dlp_templates.get_dlp_templates.return_value = (MockBox({'id': 1, 'name': 'test', 'whitelist_urls': [], 'blacklist_urls': []}), None, None)
            client_instance.dlp_templates.list_dlp_templates.return_value = ([MockBox({'id': 1, 'name': 'test', 'whitelist_urls': [], 'blacklist_urls': []})], None, None)
            yield client_instance

    def test_list_or_get(self, mock_client):
        mock_client.dlp_templates.list_dlp_templates.return_value = ([MockBox({'id': 1, 'name': 'test', 'whitelist_urls': [], 'blacklist_urls': []})], None, None)
        set_module_args(provider=DEFAULT_PROVIDER)
        from ansible_collections.zscaler.ziacloud.plugins.modules import zia_dlp_notification_template_info
        with pytest.raises(AnsibleExitJson) as result:
            zia_dlp_notification_template_info.main()
