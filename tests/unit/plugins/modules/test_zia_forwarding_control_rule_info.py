...
class TestForwardingControlRuleinfoModule(ModuleTestCase):
    @pytest.fixture
    def mock_client(self):
        with patch("ansible_collections.zscaler.ziacloud.plugins.modules.zia_forwarding_control_rule_info.ZIAClientHelper") as mock_class:
            ...
            client_instance.forwarding_control.list_rules.return_value = ([MockBox({'id': 1, 'name': 'test', 'whitelist_urls': [], 'blacklist_urls': []})], None, None)
            yield client_instance

    def test_list_or_get(self, mock_client):
        mock_client.forwarding_control.list_rules.return_value = ([MockBox({'id': 1, 'name': 'test', 'whitelist_urls': [], 'blacklist_urls': []})], None, None)
        set_module_args(provider=DEFAULT_PROVIDER)
        from ansible_collections.zscaler.ziacloud.plugins.modules import zia_forwarding_control_rule_info
        with pytest.raises(AnsibleExitJson) as result:
            zia_forwarding_control_rule_info.main()
